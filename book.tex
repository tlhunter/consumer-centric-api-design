...
\href{http://thomashunter.name}{Thomas} previously worked as a Developer Advocate/API Architect of a large filesharing and storage service, where his main concern was getting a well-documented API into the hands of third-party developers. While working at a company in the financial industry he evangelized an internal API facade to abstract backend disparity. He's also given verious talks on the topic of API Design. His previous book \href{http://amzn.to/1aqVRvq}{Backbone.js Application Development} guides the reader through the process of building an API-consuming JavaScript application.
...
I'd also like to make a special thanks to \href{https://twitter.com/thecodeplanet}{Jon} who once told me, "Tom, you should turn that \href{http://codeplanet.io/principles-good-restful-api-design/}{API blog post} into a book."
...
\subsection{Common Root API Locations}
...
The Stripe API \cite[\#Errors]{STRIPE} makes great use of error strings for conveying error codes. One such example is \texttt{expired\_card}, which as a third-party developer, you immediately know that the user-supplied card has expired.
...
There are also the OAuth 1.0a \cite{RFC5849} and xAuth \cite{XAUTH} standards, which solve mostly the same problems. Whichever method you do choose, ensure it is trustworthy and well-documented, having many different libraries written for the languages and platforms which your Consumers will likely be using.
...
\subsection{Provide Example cURL Commands}
...
This is the example cURL command displayed on the Mailgun homepage. The provided API key is even functional, so by pasting this command into a terminal, a developer can immediately make real API calls.
